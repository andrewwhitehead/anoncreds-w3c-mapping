"""Convert an AnonCreds presentation into (draft) W3C Credential format."""
...
def encode_credentials(req_json: dict, pres_json: dict) -> list:
    ...
    for reft, group in req_proof.get("revealed_attr_groups", {}).items():
        ...
        for name, attr in group["values"].items():
            ...
            creds[idx]["attribute"].append({"name": name, "value": attr["raw"]})
            mapping.append(name)
    ...
    for reft, group in req_proof.get("unrevealed_attrs", {}).items():
        ...
        if not isinstance(req_attr_map[reft], str):
            raise Exception(f"Expected single name for attribute referent: {reft}")
        creds[idx]["mapping"].setdefault("unrevealedAttributes", {})[
            reft
        ] = req_attr_map[reft]
    ...
    for reft, group in req_proof.get("predicates", {}).items():
        ...
        creds[idx]["mapping"].setdefault("requestedPredicates", {})[reft] = {
            "index": pred_idx,
    ...
    if self_attest:
        creds[idx]["mapping"]["selfAttestedAttributes"] = self_attest
...
def decode_eq_proof(eq_proof: str, subject: dict) -> dict:
    entries = {
        "revealed_attrs": {
            ...
            for attr in subject.get("attribute", [])
        }
    }
...
def decode_aggregated_proof(cred_json: dict) -> str:
    ...
    pres_proof = base64_decode(cred_json["proof"]["aggregated"])
    while pres_proof:
        pfx = pres_proof[0]
        if pfx == 0:
...
def decode_credential_proof(cred_json: dict) -> dict:
    ...
    for idx, pres_proof in enumerate(cred_json["proof"]["credential"]):
        ...
        attrib_map = {attr["name"]: attr["value"] for attr in subj.get("attribute", [])}
